...
void APlayerCharacter::WallRun(AActor* HitActor, FVector HitImpactNormal)
{
	if(!bIsWallRunning)
	{
		...
		if(CanDoWallRun(HitActor,HitImpactNormal))
		{
			...
			EWallRunDirection LWallRunSide = WallAngleWithPlayer > 0.f? 
...
{	
	...
	GetWorld()->GetTimerManager().SetTimer(LTimerHandle,[&](){bCanDoWallRunAgain = true;},WallRunReEnableTime,false);
...
bool APlayerCharacter::CanUncrouch(float CapsuleHeightAlpha) const
{
	...
	FVector TraceEnd = FVector(0,0,(StandingCapsuleHeight + 
							  (StandingCapsuleHeight - CapsuleHeightAlpha - TraceRadius))) * 
							  GetActorUpVector() + GetActorLocation();
	TArray<AActor*> ActorsToIgnore;
	FHitResult OutHit;
	...
	if(bIsPickupHit)
	{
		...
		if(HitActor != nullptr)
		{
			...
			if(PickupWeaponInterface != nullptr)
			{
				...
				{
					if(UKismetMathLibrary::NotEqual_ClassClass(PrimaryWeapon.PickupWeaponClass.Get(),HitActor->GetClass()) && 
					UKismetMathLibrary::NotEqual_ClassClass(SecondaryWeapon.PickupWeaponClass.Get(),HitActor->GetClass()))
					{
						if(bCanPickup)
							{
								bCanPickup = false;
					...
					else//Weapon Already in the slot
					{
						...
						if(UKismetMathLibrary::EqualEqual_ClassClass(PrimaryWeapon.PickupWeaponClass.Get(),HitActor->GetClass()))
						{
						...
						else if (UKismetMathLibrary::EqualEqual_ClassClass(SecondaryWeapon.PickupWeaponClass.Get(),HitActor->GetClass()))
						{
...
void APlayerCharacter::SetWeaponVars(FWeaponData NewWeaponData,bool bIsPrimaryWeapon)
{
	...
	if(NewWeaponData.WeaponClass != nullptr)
	{
		...
		CurrentWeapon->TotalAmmo 		= NewWeaponData.TotalAmmo;
		CurrentWeapon->MagSize 			= NewWeaponData.MagSize;
		CurrentWeapon->CurrentMagAmmo 	= NewWeaponData.CurrentMagAmmo;
		CurrentWeapon->FireRate 		= NewWeaponData.FireRate;
...
void APlayerCharacter::DropWeapon()
{
	if(WeaponEquippedSlot == 0)
	{
		APickupWeaponBase* DroppedWeapon = GetWorld()->SpawnActor<APickupWeaponBase>(PrimaryWeapon.PickupWeaponClass,GetActorLocation(),GetActorRotation());
		DroppedWeapon->MaxAmmo = PrimaryWeapon.MaxAmmo;
		DroppedWeapon->CurrentMagAmmo = PrimaryWeapon.CurrentMagAmmo;
		DroppedWeapon->TotalAmmo = PrimaryWeapon.TotalAmmo;
		DroppedWeapon->MagSize = PrimaryWeapon.MagSize;

	}
	else if(WeaponEquippedSlot == 1)
	{
		APickupWeaponBase* DroppedWeapon = GetWorld()->SpawnActor<APickupWeaponBase>(SecondaryWeapon.PickupWeaponClass,GetActorLocation(),GetActorRotation());
		DroppedWeapon->MaxAmmo = SecondaryWeapon.MaxAmmo;
		DroppedWeapon->CurrentMagAmmo = SecondaryWeapon.CurrentMagAmmo;
		DroppedWeapon->TotalAmmo =SecondaryWeapon.TotalAmmo;
		DroppedWeapon->MagSize = SecondaryWeapon.MagSize;
	}
}
...
void APlayerCharacter::MeleeAttack()
{
	if(!bIsDoingMeleeAttack && !bIsReloading && !bIsADS)
	{
		...
		GetWorld()->GetTimerManager().SetTimer(MeleeTimerHandle,[&](){bIsDoingMeleeAttack = false;},MeleeEndTime,false);
	}
}
